...
test("offline cache uses the current UI asset generation", async () => {
  ...
  assert.match(html, /app\.mjs\?v=53/);
  assert.match(html, /<script type="module" src="\.\/app\.mjs\?v=53"><\/script>/);
  ...
  assert.match(worker, /segref3d-lite-web-v58/);
  ...
  assert.match(worker, /app\.mjs\?v=53/);
