...
test("offline cache uses the current UI asset generation", async () => {
  ...
  assert.match(html, /app\.mjs\?v=40/);
  assert.match(html, /<script type="module" src="\.\/app\.mjs\?v=40"><\/script>/);
  assert.match(html, /TutorialSegRef3DLiteEN\.html/);
  assert.match(html, /AskAISegRef3D\.html/);
  assert.match(worker, /segref3d-lite-web-v43/);
  ...
  assert.match(worker, /app\.mjs\?v=40/);
