...
class BuildEditionTests(unittest.TestCase):
    ...
    def test_gpu_build_normalizes_dlls_and_requires_frozen_gpu_check(self):
        ...
        verification = (ROOT / "scripts" / "verify_windows_release.ps1").read_text()
        self.assertLess(package.index("sign_windows_release.ps1"), package.index("CreateFromDirectory"))
        self.assertLess(verification.index("ExtractToDirectory"), verification.index("--vtk-check"))
        self.assertIn("--gpu-check", verification)
        self.assertIn("--startup-smoke-test", verification)
        self.assertIn("prepare_windows_release.py", gpu)
        self.assertIn("ALLOW_UNSIGNED_RELEASE", package)
